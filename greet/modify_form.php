<? 
	...
	*/
	...
	$sql = "select * from greet where num=$num";  //num으로 검색하고
	$result = mysql_query($sql, $connect); 

	$row = mysql_fetch_array($result);     //fetch_array로 배열형태로   	
	$item_subject     = $row[subject];  //내용을 가져오고
	$item_content     = $row[content];  //본문글을 가져오고
?>
     ...
     <article id="content">
		...
		<ul id="write_form" class="bbs_write_top">
			...
			<li id="write_row2">
			    ...
			    <dd class="col2"><input type="text" id="subject" name="subject" value="<?=$item_subject?>" ></dd>
				</dl>
			</li>
			<li id="write_row3">
			<dl>
			<dt class="col1"><label for="content"> 내용</label></dt>
				<dd>
